...
---
I was recently tasked with migrating our applications from standalone Redis to Redis Sentinel. This involves a total of 18 apps—10 built in CodeIgniter 3 and the rest in Laravel. We had recently migrating our PHP Sessions from the default PHP session save location (file) to the database and thereafter a distributed system ([Redis](https://redis.io)), but we needed to switch over to a high availability solution ([Redis Sentinel](https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/)) in preperation for a future server patching. Today, I'd like to talk about my solution to integrating Redis Sentinel with CodeIgniter.

One of my favorite approaches when tackling a new task is to start by researching existing solutions—no need to reinvent the wheel if it's already been done! I did the same for this task, but came up empty-handed. There were virtually no resources on Redis Sentinel integration with CodeIgniter, not even in the official CI docs, as Sentinel and clustering aren't supported out of the box with CodeIgniter—well, at the time of this research. Instead, I was greeted with an unanswered legacy [Stackoverflow question](https://stackoverflow.com/questions/42209295/session-storage-with-redis-sentinel-in-php) and an answered [CodeIgniter GitHub Issue](https://github.com/bcit-ci/CodeIgniter/issues/4575) which further established how much on my own I was.

> _I’d love to dive deeper into the reasons for using [Redis](https://redis.io) and [Sentinel](https://redis.io/topics/sentinel), but I’ll leave that for you to explore so we can keep this short and focused. There are plenty of resources available to help you understand the 'why' behind this setup._

The goal was to transition to Redis Sentinel for managing both session and application data. It quickly became clear that I’d need to write a [custom session handler](https://codeigniter.com/userguide3/libraries/sessions.html#custom-drivers), a task I dreaded since the CI docs clearly warned to avoid this route because it "_is typically not an easy task, as it takes a lot of knowledge to do it properly_". Upon further research, I came across the [Predis](https://github.com/predis/predis) library, which was a huge help—it already includes support for Redis Sentinel, solving half of the challenge right there. The plan was then simple and clear - write a new session driver class that connects to Redis via Predis _since the default Redis driver did this, but via [PhpRedis](https://github.com/phpredis/phpredis)_.

> _Again, I’d love to dive deeper into the reasons for using [Predis](https://github.com/predis/predis) over [PhpRedis](github.com/phpredis/phpredis), but I’ll leave that for you to explore so we can keep this short and focused. There are plenty of resources available to help you understand the 'why' behind this choice._
